"""Task B agent — personalized recommendations.

Supports four modes through one entry point :meth:`RecommendationAgent.recommend`:

* warm-start (``user_id`` known, history-vector reuse)
* cold-start (``persona`` text only)
...
from hackathon.core.repository import fetch_item, fetch_user_history, get_user_vector
...
async def _user_vector_or_synthesize(user_id: str) -> tuple[list[float], list[dict[str, Any]]]:
    """Return (vector, training_history). Falls back to embedding history text if no cached vector."""
...
class RecommendationAgent(BaseAgent):
    ...
    async def _build_query_vector(
        ...
        """Return the persona vector and the user's already-reviewed item ids."""
        if user_id:
