...
|---|---:|---:|---:|
| Retrieval only, no language model | 30 | 0.033 | 0.017 |
| Retrieval plus language model re-rank | 30 | 0.033 | 0.033 |
| Cold start, persona text only | 20 | 0.250 | not applicable |

Three things are worth reading out of the table.

First, the language model re-rank doubles the normalised gain at parity hit rate. Both the retrieval-only and the retrieval-plus-re-rank rows hit a held-out positive in the top ten at the same three point three percent rate, but the agent ranks that hit higher in the list, exactly the explainability-aware reordering the brief asks for.
...
| Hit at ten reads low in absolute terms | The headline rate is three point three percent against a catalogue of eleven thousand items. | The number is eighteen times better than random for that catalogue size. The cold-start row at twenty five percent is the more representative quality signal. |
