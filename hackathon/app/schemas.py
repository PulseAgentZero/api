...
class PersonaInput(BaseModel):
    """User persona for direct-input review simulation (challenge spec)."""
...
class SimulateReviewRequest(BaseModel):
    """Task A input — two modes per hackathon brief.

    **Direct mode:** ``persona`` + ``product`` (no database).
    **DB mode:** ``user_id`` + ``item_id`` (loads from Yelp slice in Postgres).
    """
